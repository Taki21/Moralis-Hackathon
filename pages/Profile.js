...
export default function Profile() {
  const account = useMoralisWeb3Api();
    ...
    const { error, isUploading, moralisFile, saveFile } = useMoralisFile();
      ...
      const nfts = async () => { 
        const result = await account.getNFTs({chain: "avalanche testnet" })
        console.log(result)
      }
        ...
        /*output.forEach((item) => {
            ...
            return (
              ...
              actions={[
                  ...
                  <ShoppingCartOutlined onClick={() => handleSellClick(item.token_address)} />
                  </Tooltip>,
              ]}
              ...
              key={index}
              >
              <Meta title={item.name} description={"this is a pretty epic description"} />
              </Card>
            )
